...
- Provider、密钥安全、角色绑定和后端自动化测试；
- 完整 OpenAPI/Pydantic 契约，供当前电脑后续开发 UI。

本关禁止修改 `apps/web` 下任何文件。高保真“模型与费用设置”页面、React Query 接入、CSS、交互和 Playwright 视觉测试全部由当前电脑的 GPT-5.6 在后端审计通过后实施。
...
测试不得调用真实付费模型，必须使用内存 SecretStore 和本地假 OpenAI 服务。必须运行现有前端测试以确认没有回归，但不得修改前端代码或前端测试。
...
- 不提前实施关卡二、三、四。
- 不修改 `apps/web` 下任何文件；全部 UI 工作由当前电脑 GPT-5.6 负责。
