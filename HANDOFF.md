...
状态：当前电脑实施、自审计和首轮实机 UI 巡检完成，等待用户配置本地密钥并确认单章流程；未合并 `main`
...
1. 已在本地实机检查 `/canon`、`/overview` 和 `/automation`：无浏览器错误；根地址现默认进入试写引导，页面标题已改为 `Story Agent · 墨曜指挥舱`。
2. 本机已创建 DeepSeek 官方预设；用户需在“模型与费用设置”本人输入 API Key、保存并点击连接测试，再完成六个自动写作角色绑定。
...
- 实机入口修复后复验：Web `3 files / 11 tests passed`，`npm run build` 通过。
