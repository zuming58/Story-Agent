...
- 修复“生成 3—5 个方向”容易 `MODEL_TIMEOUT`：调用不再发送完整证据摘录与项目元数据，只发送可审计的证据 ID、短 claim、类型、置信度和研究发现；要求固定生成三条短候选，将输出上限从默认 `8192` 收紧为 `2800` tokens，并关闭这一步的隐式 Provider 重试，避免用户一次点击连续等待两轮超时。证据 ID、评分上限、revision 与人工选择门槛保持不变。
...
- `git diff --check`：通过。
- API 专项：`test_story_opportunities_use_a_compact_model_snapshot` 通过，验证紧凑输入快照、`2800` token 上限和无重试。
