...
class _FakeModelHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):  # noqa: N802
        ...
        step = next((name for name in ("query_plan", "evidence", "research_report", "external_directions", "opportunity_repair", "opportunities", "ideation_kickoff_world", "ideation_kickoff_characters", "ideation_kickoff_boundaries", "ideation", "story_brief", "canon_analyze", "canon_repair", "canon", "opening_expand", "opening", "opening_review") if f'"phase14Step":"{name}"' in joined or f'"phase14Step": "{name}"' in joined), "")
        ...
        elif step == "ideation_kickoff_world": value = {"title": "A provisional world", "premise": "A choice begins the story.", "worldView": "A constrained world with a visible cost.", "firstThreeChapterPromise": "A choice creates a public consequence.", "serialEngine": "Each answer creates a harder choice."}
        elif step == "ideation_kickoff_characters": value = {"characterCandidates": ["Ming, early career, wants the truth."], "relationshipCandidates": ["Ming and a rival need each other."], "rulesAndResources": "Status and evidence carry a cost; no fantasy power system.", "coreConflicts": ["Truth costs trust."]}
        elif step == "ideation_kickoff_boundaries": value = {"writingBoundaries": ["No imitation."], "openQuestions": ["How public is the first loss?", "Which relationship hurts most?"]}
...
def test_research_to_opening_selection_is_isolated_and_deterministic(client, monkeypatch):
    ...
    kickoff = client.post(f"/api/v1/ideation/sessions/{session['id']}/kickoff", json={"expectedSessionRevision": session["revision"]})
    assert kickoff.status_code == 201, kickoff.text
    assert kickoff.json()["structuredState"]["kind"] == "co_creation_kickoff"
    assert "全部待你确认" in kickoff.json()["content"]
    restored_session = client.get(f"/api/v1/projects/{project['id']}/ideation/sessions/{session['id']}").json()
    assert restored_session["revision"] == session["revision"] + 1
    ...
    assert {"research_planner:query-plan", "research_analyst:report", "story_incubator:opportunities", "story_incubator:ideation-kickoff:world", "story_incubator:ideation-kickoff:characters", "story_incubator:ideation-kickoff:boundaries", "story_incubator:story-brief", "story_incubator:canon", "research_analyst:canon-analyzer", "reader_simulator:opening-review", "opening_editor:opening-review"}.issubset(roles)
    incubator_calls = [item for item in calls if item["run_role"].startswith("story_incubator:")]
    assert {"story_incubator:ideation", "story_incubator:ideation-kickoff:world", "story_incubator:ideation-kickoff:characters", "story_incubator:ideation-kickoff:boundaries", "story_incubator:story-brief", "story_incubator:canon", "story_incubator:opening-expand"}.issubset({item["run_role"] for item in incubator_calls})
