...
export function AgentPanel() {
  ...
  const submit = async (event?: FormEvent, quickPrompt?: string, action = "chat") => {
    ...
    if (!raw || thinking || (!selected && !contextualMode)) return;
    const scopePrefix = contextualMode && agentScopeLabels.length ? `当前作用域：${agentScopeLabels.join(" / ")}\n\n` : "";
    const content = writingMode && agentSelection
      ? `${scopePrefix}${raw}\n\n当前正文选区（只可建议修改，不得直接写入正式状态）：\n${agentSelection}`
      : `${scopePrefix}${raw}`;
    setInput("");
    setThinking(true);
    try { await sendMessage(content, action, contextualMode ? null : undefined); } finally { setThinking(false); }
  };
      ...
      <div className="agent-scroll-region">
        ...
        <section className="quick-actions" aria-label="AI 快捷操作">
          {writingMode ? <>
            ...
            <button onClick={() => void submit(undefined, "请检查当前 Canon 作用域中的设定冲突、同名歧义和缺失约束，只给出建议。") } disabled={thinking}><ShieldCheck size={16} />检查设定</button>
            <button onClick={() => void submit(undefined, "请列出当前实体可能缺失的关系、能力边界和一致性字段。") } disabled={thinking}><GitBranch size={16} />补全关系</button>
            <button onClick={() => void submit(undefined, "请解释当前规则对后续章节契约与质量门的影响。") } disabled={thinking}><BookBookmark size={16} />解释边界</button>
          </> : automationMode ? <>
