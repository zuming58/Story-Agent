...
export function ProjectOverviewPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setOpen(false); setTitle(""); selectProject(created.id); navigate("/canon");
  };
  ...
  return <div className="project-overview-page">
    ...
    <section className="project-grid">{projects.map((item) => <button key={item.id} className={`project-card${project?.id === item.id ? " is-active" : ""}`} onClick={() => { selectProject(item.id); navigate("/planning"); }}><div className="project-card-icon"><BookOpen size={28} weight="duotone" /></div><div><strong>{item.title}{item.projectKind === "demo" && <em className="project-kind-badge">示例·从第36章开始</em>}</strong><span>{item.mode === "long-form" ? "长篇网文" : item.mode === "short-form" ? "短篇小说" : "短剧项目"} · {item.currentChapter}/{item.totalChapters} 章</span><small><FolderOpen size={13} />{item.folderPath}</small></div></button>)}</section>
  </div>;
}
