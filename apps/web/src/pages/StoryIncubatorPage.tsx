...
export function StoryIncubatorPage() {
  ...
  const generateOpportunities = async (creativeInput?: string) => {
    if (!currentJob) return;
    await run(creativeInput ? "已根据外部创意结果生成故事机会" : "已生成故事机会", () => api.createStoryOpportunities(currentJob.id, currentJob.revision, creativeInput));
  };
  const importExternalCreativeInput = async () => {
    const content = externalCreativeInput.trim();
    if (!content) return;
    try {
      await generateOpportunities(content);
      setExternalCreativeInput("");
      setExternalCreativeInputOpen(false);
    } catch {
      // The shared mutation already exposes the API error and preserves the input for retry.
    }
  };
  ...
  return <div className="incubator-page">
    ...
    <section className="incubator-workspace">
      ...
      {stage === 2 && <div className="incubator-panel opportunity-deck"><header><div><Sparkle /><span><strong>故事机会候选</strong><small>这里只选方向，完整设定留到人机共创</small></span></div>{currentJob?.status === "accepted" && !opportunitiesQuery.data?.length && <div className="opportunity-actions"><button onClick={() => setExternalCreativeInputOpen((open) => !open)} disabled={mutation.isPending}><Books />导入外部创意</button><button className="gold-action" onClick={() => void generateOpportunities()} disabled={mutation.isPending}>{mutation.isPending ? <><CircleNotch className="spin" />正在生成方向…</> : <><Sparkle />生成 3 个方向</>}</button></div>}</header>{externalCreativeInputOpen && <section className="external-opportunity-input"><header><div><Books /><span><strong>导入外部创意结果</strong><small>只作为创作偏好，不替代市场证据或自动确认</small></span></div><button aria-label="关闭外部创意输入" onClick={() => setExternalCreativeInputOpen(false)}><X /></button></header><textarea value={externalCreativeInput} onChange={(event) => setExternalCreativeInput(event.target.value)} placeholder="粘贴你和其他 Agent 打磨后的完整结果：题材融合、核心冲突、人物关系、世界观取舍、想保留或排除的元素都可以写。系统会基于已接受的研究证据整理成三张可选择的方向卡。" /><footer><small>{externalCreativeInput.trim().length}/30000 · 不会作为研究事实写入证据链</small><button className="gold-action" onClick={() => void importExternalCreativeInput()} disabled={mutation.isPending || externalCreativeInput.trim().length < 40}>{mutation.isPending ? <><CircleNotch className="spin" />正在整理…</> : <><Sparkle />从输入生成方向</>}</button></footer></section>}<div className="opportunity-grid">{opportunitiesQuery.data?.map((item, index) => <article key={item.id} className={item.status === "accepted" ? "is-accepted" : ""}><header><span>方向 {String(index + 1).padStart(2, "0")}</span><strong>{item.totalScore}<small>/100</small></strong></header><h2>{String(item.story.title ?? item.highConcept)}</h2><p className="opportunity-summary">{String(item.story.summary ?? item.highConcept)}</p><footer><span>{Math.round(item.evidenceCoverage * 100)}% 证据覆盖</span>{item.status === "pending" && <div><button onClick={() => decideOpportunity(item.id, item.revision, "reject")}><X />排除</button><button className="accept" onClick={() => void decideOpportunity(item.id, item.revision, "accept")}><Check />选这个方向</button></div>}{item.status === "accepted" && <b><CheckCircle />当前方向</b>}</footer></article>)}</div>{!opportunitiesQuery.data?.length && <div className="incubator-empty"><Lightbulb /><strong>先接受研究报告，再让模型基于证据提出真正不同的故事方向。</strong></div>}</div>}
