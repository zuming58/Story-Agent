...
结论：代码级实施、真实 Canon/规划和第 1—5 章真模型验收均已完成，等待用户 UI 与正文体验确认。
...
## 2026-07-14 第 1—5 章最终验收

### 结果

- 恢复第一章原任务和 v3 安全候选，未重新调用写作模型，完成质量门与原子提交。
- 第二章至第五章逐章生成、复核和正式提交，项目进度从 0 准确推进到 5。
- 第三、四章各使用两轮修订，没有产生第三轮；第五章首稿无阻断项。
- 每章只有一个 current official commit；候选历史仍完整保留。
- 服务重启后 `currentChapter=5`、五章 commit、Canon、状态快照和本地章节镜像均恢复正确，且没有残留 active job。

### 新发现与修复

1. 后端已有恢复能力但没有公共手动恢复路由；新增 `/chapter-jobs/{job_id}/resume` 并验证复用现有候选。
2. 确定性规则升级后缺少安全重检入口；新增带 revision 保护的 `/quality/revalidate`，旧误报保留历史但不继续阻断。
3. 规划条件和自然正文的中文表达差异产生误报；补充受控语义证据、两次观察计数和描述性人物称谓匹配，不放宽普通人名。
4. 真实抽取出现输出膨胀与非数值 confidence；压缩四类抽取请求、限制重试并规范 confidence。
5. 写作和修订提示未充分强调契约字数与边界；补充字数、人物、钩子、伏笔、完成条件和禁写范围自检。
...
结论：第八阶段达到“可进行 3—5 章真实连续试写”的验收线。下一步先由用户检查正文与 UI，再决定合并 PR #8 和启动第九阶段导出能力。
