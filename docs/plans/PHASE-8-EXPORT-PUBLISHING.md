...
状态：等待另一台电脑实施，完成后交回当前 GPT-5.6 审计
建议分支：`agent/export-publishing-foundation`
范围所有权：另一台电脑只开发后端、数据库、文件渲染、公共 API 类型和 API 测试；禁止修改 `apps/web/**`
...
新增项目迁移 `0010_export_publishing_foundation`（如前序审计新增迁移导致编号变化，以实际 head 顺延）：
...
1. 从第七阶段审计合并后的最新 `main` 创建 `agent/export-publishing-foundation`。
...
第八阶段审计通过后，由当前电脑独占开发自动托管与导出控制台 UI。
